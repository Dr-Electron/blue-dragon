...
pub fn run(opts: Opt) {
    ...
    while running.load(Ordering::SeqCst) {
        ...
        let resp = client
            ...
            .send()
            .expect("Error sending Asset request");
        if !resp.status().is_success() {
            match resp.status() {
                reqwest::StatusCode::FORBIDDEN | reqwest::StatusCode::NOT_FOUND => {
                    //Asset registry specific errors
                    if resp.headers()[reqwest::header::CONTENT_TYPE]
                        .to_str()
                        .unwrap()
                        .contains(&"json")
                    {
        ...
        let resp = client
            .get(format!("{}/admin/filters", opts.asset_domain()))
            .basic_auth(opts.user(), Some(opts.password()))
            .send()
            .expect("Error requesting filters");
        if !resp.status().is_success() {
            match resp.status() {
